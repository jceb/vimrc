-- Source: https://raw.githubusercontent.com/phelipetls/dotfiles/master/.config/nvim/lua/tsconfig.lua
local M = {}
...
function M.get_tsconfig_paths(tsconfig_fname, prev_base_url)
    ...
    local base_url = json.compilerOptions and json.compilerOptions.baseUrl
        or prev_base_url
        or ""
    ...
    if json.compilerOptions and json.compilerOptions.paths then
        for alias, paths in pairs(json.compilerOptions.paths) do
            alias_to_paths[alias] = vim.tbl_map(function(path)
                return vim.fn.simplify(
                    get_dir(tsconfig_fname)
                        .. "/"
                        .. base_url
                        .. "/"
                        .. path:gsub("*", "")
    ...
    return vim.tbl_extend(
        "force",
        alias_to_paths,
        M.get_tsconfig_paths(tsconfig_extends, base_url)
    )
end
...
local function get_tsconfig_include(tsconfig_fname)
    ...
    local include_list = vim.tbl_map(function(fname)
        ...
        local dir = vim.fn.simplify(
            get_dir(tsconfig_fname) .. "/" .. fname_without_globs
        )
    ...
    for alias, paths in pairs(alias_to_paths) do
        if alias == "*" or vim.startswith(input, alias:gsub("*", "")) then
            for _, path in pairs(paths) do
                ...
                local real_path = find_file(expanded_path)
                    or find_dir(expanded_path)
